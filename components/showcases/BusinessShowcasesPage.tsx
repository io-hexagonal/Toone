...
export async function generateBusinessShowcasesMetadata({ params }: Props): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "showcases" });

  const url = `https://trytoone.com/${locale}/business/showcases`;

  const languages: Record<string, string> = {};
  for (const l of locales) {
    languages[l] = `https://trytoone.com/${l}/business/showcases`;
  }
...
export default async function BusinessShowcasesPage({ params }: Props) {
  const { locale } = await params;
    ...
    <>
      ...
      <div className="sc-page">
        ...
        <section className="sc-cta">
          ...
          <h2>{t("ctaTitle")}</h2>
          <p>{t("ctaSub")}</p>
          <ContactCta className="sc-btn" source="business-showcases">
            {t("ctaBtn")}
          </ContactCta>
          <div>
            <Link className="sc-back" href="/business">
              {t("backHome")}
            </Link>
          </div>
        </section>
