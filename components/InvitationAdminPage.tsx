              ...
              <section className={styles.createGrid} aria-label="Create and share an invitation">
                ...
                <div className={`${styles.panel} ${styles.share}`} aria-live="polite">
                  {created ? <>
                    ...
                    {created.email ? <p className={styles.notice}>{created.email_queued ? `Email queued for ${created.email}. Keep the link below in case you need to share it yourself.` : `Email is not enabled on the server, so nothing was sent to ${created.email}. Send the message below yourself.`}</p> : null}
