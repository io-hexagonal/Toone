...
test("prices parse leniently: anything missing or malformed is free", () => {
...
test("public entries, details and bundle members carry a normalized price", async (t) => {
  ...
  t.mock.method(globalThis, "fetch", async (input: string | URL) => {
    ...
    const data = url.includes("/bundles/") ? bundle : url.includes("/workflows/") ? routine : [entry];
  ...
  assert.deepEqual(list.items[0].price, FREE);
});
