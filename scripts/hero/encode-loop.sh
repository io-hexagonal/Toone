...
# The poster is the loop's first frame, so the page can crossfade poster → video
...
ffmpeg -v error -y -i "$OUT_DIR/glitter-loop.mp4" -frames:v 1 -q:v 3 "$OUT_DIR/glitter-poster.jpg"
ls -la "$OUT_DIR"
